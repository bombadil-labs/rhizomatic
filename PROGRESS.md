...
>
> - **The experiment ran and the suite mostly held.** `implementations/elixir` (106 tests, zero
>   deps — pure-Elixir BLAKE3 + Edwards arithmetic, `:crypto` for signing only) was written in a
>   fresh context forbidden from reading the other witnesses. Verdict: everything at L0 was
>   implementable from spec text with vectors as confirmation — **except the pack layout**, where
>   SPEC-8 §3's grammar could not reproduce `packHex` (the vector's bytes were the real spec).
>   Nine findings, F1–F9, in `implementations/elixir/FINDINGS.md`.
> - **Findings folded back**: the missing `pack-bytes.json` vector generated at last (F1, promised
>   since 0.4 — all three witnesses consume it, and Elixir's m/y path, written from prose alone,
>   matched the generated bytes first try); SPEC-8 §3 gained the `"i"` field, the strings-table
>   sort order, and the intern enumeration (F2–F4, ERRATA-8 P4); envelopes-win pinned (F6, P5);
>   `dt` lossiness recorded open (F5, P6); README drift fixed (F7); the JSON-parser bring-up note
>   added (F8); D15 census updated (F9).
> - **Infrastructure**: N-witness `tools/check-all.mjs` driven by `implementations/*/witness.json`
>   manifests (machine-readable conformance level); CI gained an Elixir job (OTP 27 / Elixir 1.18
>   minimums); lockstep now binds per level (CLAUDE.md); the L0 bring-up path lives in
>   `vectors/README.md`; SPEC-1 §4.1 + D14 pin the host-boundary numeric policy (reject native
>   integer terms; JSON profile is the blessed coercion point) with the `number-integer-spelling`
>   vector.
> - **Open on Myk**: ERRATA D15 (whose Unicode tables answer NFC — three candidates on issue #19).
>   Known caveat: all models available to the build were Claudes, so the ticket's different-model-
>   family mitigation was only partially honored; vector failures (F1–F7) carry the evidence.
>
> All 3 witnesses green — Elixir (L0), Rust (L4), TypeScript (L4). `node tools/check-all.mjs` clean.
>
> ---
...
- **Slice I — Chorus extracted to apps/chorus; README refresh.** ✅ — Chorus is now its own
  ...
  the witness as an ordinary npm dependency (`@bombadil/rhizomatic: file:../../implementations/ts`;
  ...
  npm dep) needs only publishing @bombadil/rhizomatic — the seam is already cut.

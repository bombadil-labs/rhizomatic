...
(() => {
  ...
  function eddsa(Point2, cHash, eddsaOpts = {}) {
    ...
    function verify(sig, msg, publicKey, options = verifyOpts) {
      ...
      let A2, R, SB;
      try {
        A2 = Point2.fromBytes(publicKey, zip215);
  ...
  function withParseError(raw, parse) {
    try {
      return parse();
    } catch (cause) {
  ...
  function asRecord(x, what) {
    if (typeof x !== "object" || x === null || Array.isArray(x)) {
      throw new ParseError("invalid-shape", `expected object for ${what}`, x);
    }
    return x;
  }
  function asObject(x, what, known) {
    ...
    for (const key of Object.keys(o)) {
      if (!known.includes(key))
        throw new ParseError("unknown-key", unknownKeyMessage(key, known, what), x, key);
    }
    return o;
  }
  ...
  function oneTag(x, tags, what) {
    ...
    if (present.length === 0) {
      throw new ParseError("invalid-shape", `${what}: must be one of ${tags.join(" | ")}`, x);
    }
    if (present.length > 1) {
      throw new ParseError(
        "invalid-shape",
  ...
  function parseTarget(raw) {
    ...
    if (present.length > 1) {
      throw new Error(
        `target is ambiguous \u2014 ${present.map((p) => `"${p}"`).join(" and ")} are both present, but exactly one names the target kind`
      );
    }
    if (present[0] === "id") {
...
` + (same ? "\u2713 byte-identical. Order never matters." : "\u2717 DIVERGED \u2014 file a bug, this is a P0");
  ...
  function runRustConformance(rust) {
    ...
    rustEval("evaluator: resolve (schemas \u2192 Views)", eval_resolve_default);

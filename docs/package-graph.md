...
`src/index.ts` remains the public entry point.
...
| `eval → resolution` | Put terminal `resolve` composition in `resolve`, above the algebra evaluator. |
| `eval → schema.SchemaRegistry` | The algebra evaluator consumes a registry interface; `schema` supplies it. |
...
## Target build DAG for the boundary move
...
syntax       → delta
schema       → syntax, delta
algebra      → schema, syntax, delta
resolve      → algebra, syntax, delta
schema-load  → resolve, schema, syntax, delta
reactor      → resolve, algebra, schema, syntax, delta
storage      → delta
federation   → reactor, resolve, algebra, syntax, delta
...
the terminal `resolve` term composition and public `evalTerm` entry point; `algebra` owns the
underlying evaluation. This prevents either package from importing upward. Future `principal`
...
The graph check treats type-only imports as package edges. It permits internal source cycles
within a package but rejects cycles across packages. The aggregate public barrel is excluded
as an entry point and cannot be imported by an internal package.
